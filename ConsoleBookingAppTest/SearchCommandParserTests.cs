...
public class SearchCommandParserTests
{
    ...
    private static IEnumerable<TestCaseData> ParserTestNegativeCases()
    {
        ...
        string[][] p1 = [[" A B ", testCaseDaysAsString[0], "Standard "]];
        yield return new TestCaseData(p1).SetName("Parse_Throws_SearchCommandHandlerParseException1");

        string[][] p2 = [["D E", testCaseDaysAsString[1], "Deluxe"]];
        yield return new TestCaseData(p2).SetName("Parse_Throws_SearchCommandHandlerParseException2");

        string[][] p3 = [["G666HI", testCaseDaysAsString[2], "Standard"]];
        yield return new TestCaseData(p3).SetName("Parse_Throws_SearchCommandHandlerParseException3");

        string[][] p4 = [["101   0101    010", testCaseDaysAsString[3], " S Standard"]];
        yield return new TestCaseData(p4).SetName("Parse_Throws_SearchCommandHandlerParseException4");

        string[][] p5 = [[" AB ", testCaseDaysAsString[4], "Sta         ndard"]];
        yield return new TestCaseData(p5).SetName("Parse_Throws_SearchCommandHandlerParseException5");
    }
}
